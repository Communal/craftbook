...
export const LikeButton = forwardRef<HTMLButtonElement, ButtonHTMLAttributes<HTMLButtonElement> & LikeButtonProps>(
  ({ className, count = 0, ...props }, ref) => {
    ...
    return (
      <span className="flex flex-row items-center gap-0.5 w-fit h-fit">
        <button
          ref={ref}
          className={cn("p-1 rounded-full hover:bg-black/5 active:bg-black/10", className)}
...
export const ShareButton = forwardRef<HTMLButtonElement, React.ButtonHTMLAttributes<HTMLButtonElement>>(
  ({ className, ...props }, ref) => {
    return (
      <>
        <DropdownMenu>
