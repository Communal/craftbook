...
export const CommunityCard = forwardRef<HTMLDivElement, CommunityCardProps>(
  (
    { className, children, logo, title, description, communityName, ...props },
    ref,
  ) => {
    return (
      <Card
        ref={ref}
        className={cn(
          'community-card w-[260px] h-[124px] hover:scale-95 hover:cursor-pointer transition-all',
          className,
        )}
        {...props}>
        ...
        <p className="text-sm text-neutral-500 leading-snug mt-3">
          {description}
        </p>
      </Card>
    );
