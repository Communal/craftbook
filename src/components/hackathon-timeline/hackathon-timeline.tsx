...
import { Badge, Button, LinkButton } from '..';
...
>(
  (
    ...
    return (
      <div
        ref={ref}
        className={cn('hackathon-timeline grid grid-cols-1 gap-6 border-l', className)}
        {...props}
      >
        ...
        <div className="pl-12">
          ...
          <div className='hackathon-timeline-event-meta-details-wrapper mt-2 mb-4'>
            <div className='hackathon-timeline-event-timestamp-wrapper text-xs font-medium text-neutral-500'>
              {`(${new Date(startAt).getHours()}:${new Date(startAt).getMinutes()}) ${new Date(startAt).getDate()} ${getMonthNameByIndex(new Date(startAt).getMonth())?.short}, ${new Date(startAt).getFullYear()}`}
              {endsAt && " - "}
              {endsAt
                && `(${new Date(endsAt).getHours()}:${new Date(endsAt).getMinutes() === 0 ? "00" : new Date(endsAt).getMinutes()}) ${new Date(endsAt).getDate()} ${getMonthNameByIndex(new Date(endsAt).getMonth())?.short}, ${new Date(endsAt).getFullYear()}`
              }
            </div>
          </div>
          <div className='my-2'>
...
export const HackathonTimelineEventTitle: React.FunctionComponent<React.HTMLAttributes<HTMLHeadingElement>> = ({
  className,
  ...props
}) => {
  return <h2 className={cn("text-xl font-semibold mb-2", className)} {...props} />
}

export const HackathonTimelineEventDescription: React.FunctionComponent<React.HTMLAttributes<HTMLParagraphElement>> = ({
  className,
  ...props
}) => {
