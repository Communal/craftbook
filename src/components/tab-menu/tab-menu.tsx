...
interface TabMenuOptionProps extends React.ButtonHTMLAttributes<HTMLButtonElement> {
  title: string;
  label: string;
  isSelected: boolean;
}
...
const TabMenu = forwardRef<HTMLDivElement, TabMenuProps>(
  ({ className, options, currentOption = "", ...props }, ref) => {
    ...
    return (
      <button
        ...
        className={cn(
          ...
          isSelected && "border-b-2 border-b-black drop-shadow-lg before:outline-none",
          className
        )}
        aria-label={label}
        {...props}
